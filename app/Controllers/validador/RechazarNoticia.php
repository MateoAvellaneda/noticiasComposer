...
class RechazarNoticia extends BaseController
{
    ...
    public function rechazarNoticia(){
        $this->checkSession();
        $rechazo = $this->request->getPost();
        if(!isset($rechazo['idNoticia'])){
            return $this->response->redirect(site_url('/noticiasValidar'));
        }elseif(empty($rechazo['idNoticia'])){
            return $this->response->redirect(site_url('/noticiasValidar'));
        }
        $noticia = $this->noticiasModel->find($rechazo['idNoticia']);
        if (empty($noticia)) {
            return $this->response->redirect(site_url('/noticiasValidar'));
        } elseif ($noticia['estado'] != 'validar') {
            return $this->response->redirect(site_url('/noticiasValidar'));
        } else{
            $this->createMotivoRechazo($rechazo['motivo'], $noticia['ID']);
            $this->editDatabase($noticia['ID']);
            return $this->response->redirect(site_url('/noticiasValidar'));
        }
    }

    private function createMotivoRechazo($motivo, $idNoticia){
        $data = ['IDnoticia' => $idNoticia,
                'motivo' => $motivo
    ];
        $this->rechazosModel->insert($data, false);
    }
